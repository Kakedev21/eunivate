...
const TaskDetailModalMem = ({
  ...
  const [showSaveAttachmentButton, setShowSaveAttachmentButton] =
    useState(false);
  const [attachmentPreviewUrls, setAttachmentPreviewUrls] = useState(
    task.attachment.map((att) => att.url)
  );
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        `https://eunivate-jys4.onrender.com/api/users/sa-tasks/${task._id}/comments`
      );
  ...
  const updateTaskInDatabase = async (changes) => {
    try {
      ...
      const response = await fetch(
        `https://eunivate-jys4.onrender.com/api/users/sa-tasks/${task._id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleRemoveObjective = (objectiveToRemove) => {
    ...
    const updatedObjectives = task.objectives.filter(
  ...
  const handleCommentSubmit = async () => {
    if (comment.trim()) {
      const assignee =
        editedAssignees && editedAssignees.length > 0
          ? editedAssignees[0]
          : null;
      const user = JSON.parse(localStorage.getItem("user"));
      const currentUserId = user?._id;
    ...
    >
      <div className="bg-white rounded-lg shadow-lg p-6 w-96 h-[100vh] max-h-[100vh] overflow-y-auto relative">
        ...
        <div className="mb-4">
          ...
          >
            <span className="text-gray-700 text-sm font-semibold flex justify-normal">
              Assignees
            </span>
        ...
        <div className="mb-4 flex items-center text-gray-500 relative">
          ...
          <div
            className="ml-9 flex items-center cursor-pointer"
            onClick={handlePriorityClick}
          >
            <FaFlag className={getPriorityColor(selectedPriority)} />
            <span className={`ml-2 ${getPriorityColor(selectedPriority)}`}>
            ...
            <div className="absolute top-10 left-20 bg-white border border-gray-300 shadow-lg rounded-md w-40 z-10">
              <ul>
                {["easy", "medium", "hard"].map((priority) => (
                  <li
                    key={priority}
                    className={`flex items-center p-2 cursor-pointer hover:bg-gray-100 ${getPriorityColor(
                      priority
                    )}`}
            ...
            {commentsList.length > 0 ? (
              <ul className="list-disc list-inside">
                {commentsList.map((cmt, index) => (
                  <li
                    key={index}
                    className="mt-1 flex items-start text-gray-500"
                  >
                    <img
                      src={cmt.profilePicture?.url || cmt.profilePicture}
                      alt={cmt.username}
