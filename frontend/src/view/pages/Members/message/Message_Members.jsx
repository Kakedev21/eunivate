...
const Message_Members = ({ onInvitedUsersFetched, workspaceId }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (workspaceId) {
          ...
          const invitedUsersList = invitedUsersResponse.data.invitedUsers.map(
            (invitedUser) => {
              const userFromDB = allUsers.find(
                (user) => user.email === invitedUser.email
              );
              return userFromDB
                ? { ...invitedUser, ...userFromDB }
                : invitedUser;
            }
          );
        ...
        setError(
          "Please invite people to see the members and ensure you select a workspace."
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
      ...
      <div className="bg-gray-100 p-4 rounded-md h-52 mt-4">
        ...
        <p className="text-sm mt-2">
          All about the workspace topic related only
        </p>
        <p className="text-gray-600 text-sm mt-5">Description</p>
        <p className="text-sm mt-2">
          In this channel, we can collaborate and communicate with fellow
          members.
        </p>
      </div>
      ...
      {currentUser && (
        <div className="mt-4 flex items-center">
          ...
          <p className="ml-2 text-gray-800 text-sm font-semibold">
...
export default Message_Members;
