                        ...
                        <tbody>
                            {tasks.map((task) => (
                                <tr key={task._id} className="hover:bg-gray-100">
                                    ...
                                    {['Responsible', 'Accountable', 'Consulted', 'Informed'].map((column) => (
                                        <td key={column} className="border border-gray-200 p-2 text-sm text-gray-800">
                                                ...
                                                {selectedMembers[`${task._id}-${column}`] ? (
                                                    <div className="flex items-center space-x-2">
                                                        <img
                                                            ...
                                                            alt={`${selectedMembers[`${task._id}-${column}`].firstName} ${selectedMembers[`${task._id}-${column}`].lastName}`}
                                                            className="w-7 h-7 rounded-full object-cover"
                                                        />
                                                        <span>
                                                            {`${selectedMembers[`${task._id}-${column}`].firstName} ${selectedMembers[`${task._id}-${column}`].lastName}`}
                                                        </span>
                                                    </div>
                                                ) : (
                                                ...
                                                <div className="mt-2 max-h-40 overflow-y-auto border border-gray-300 rounded shadow-lg bg-white">
                                                    {task.assignee && task.assignee.length > 0 ? (
                                                        task.assignee.map((member, index) => (
                                                            <div
                                                                key={index}
                                                                onClick={() => selectMember(task._id, column, member)}
                                                        ...
                                                        <p className="text-gray-500 p-2">No members assigned</p>
                                                    )}
                                                </div>
                                            )}
                                        </td>
