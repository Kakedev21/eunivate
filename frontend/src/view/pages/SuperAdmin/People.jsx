...
const People = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Fetch all users
      const response = await axios.get(
        "https://eunivate-jys4.onrender.com/api/users/",
        {
          headers: {
      ...
      if (selectedWorkspace) {
        const invitedUsersResponse = await axios.get(
        ...
        const invitedUsersData = invitedUsersResponse.data.invitedUsers.map(
          (invitedUser) => {
            const userFromDB = response.data.find(
              (user) => user.email === invitedUser.email
            );
    ...
    setFilteredUsers(
      filteredByRole.filter((user) => {
        const searchQuery = document
          .querySelector('input[placeholder="Search email"]')
          .value.toLowerCase();
        return (
  ...
  const handleInvite = async () => {
    if (!selectedEmails.length || !selectedWorkspace) {
      toast.error(
        "Please select at least one email and ensure a workspace is selected.",
        {
          position: "top-right",
          autoClose: 3000,
    ...
    const selectedUsers = selectedEmails
      .map((email) => {
        ...
        return userFromDB
          ? {
              id: userFromDB._id,
              projects:
                userFromDB.projects.length > 0 ? userFromDB.projects : [],
              profilePicture: userFromDB.profilePicture || {},
    ...
    try {
      ...
      const response = await axios.post(
        "https://eunivate-jys4.onrender.com/api/users/invite",
        {
      ...
      if (response.status === 200) {
        // Assuming 200 means success in your backend
  ...
  const handleRoleChange = async (newRole, userEmail) => {
    try {
      ...
      const response = await fetch(
        `https://eunivate-jys4.onrender.com/api/users/${user._id}/role`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        alert(
          `Role changed to ${newRole} and email notification sent to ${userEmail}`
        );
      } else {
  ...
  const handleRemoveUser = async (userEmail) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://eunivate-jys4.onrender.com/api/users/invited/${invitedMember.userId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 relative">
                    ...
                    {isRoleDropdownOpen[user.email] && (
                      <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50 role-dropdown">
                        <ul className="py-1">
                          {["Guest", "members", "admin", "superadmin"].map(
                            (role) => (
                              <li
                                key={role}
              ...
              {filteredUsers.length > 0 ? (
                <ul>
                  {filteredUsers.map((user, index) => (
                    <li
                      key={index}
                      className={`flex items-center justify-between p-2 border-b text-xs sm:text-sm ${
                        selectedEmails.includes(user.email) ? "bg-gray-200" : ""
                      }`}
