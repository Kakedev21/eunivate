...
const AdminContainer = ({ children }) => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch(
          "https://eunivate-jys4.onrender.com/api/users/notifications"
        );
  ...
  const handleBellClick = async () => {
    ...
    try {
      await fetch("https://eunivate-jys4.onrender.com/api/users/notification", {
        method: "GET",
      });
    } catch (error) {
      console.error("Failed to fetch notifications", error);
    ...
    <div className="min-h-screen bg-gray-100 p-6 sm:p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center">
          ...
          <h1
            className="hidden sm:block text-3xl sm:text-4xl font-bold text-red-800 cursor-pointer mb-6 mr-10"
        ...
        <h1
          className="block sm:hidden text-3xl sm:text-4xl font-bold text-red-800 cursor-pointer mb-6 mr-0"
          onClick={() => navigate("/admin")}
        >
          Admin
        </h1>
        ...
        <div className="hidden sm:flex flex-col items-center">
          <div className="flex gap-4 bg-gray-200 rounded-full p-2 mb-4">
            <div
              onClick={() => navigate("/admin")}
              className={tabStyle(selectedTab === 0)}
            >
              Dashboard
            </div>
            <div
              onClick={() => navigate("/products")}
              className={tabStyle(selectedTab === 1)}
            >
              Products
            </div>
            <div
              onClick={() => navigate("/projects")}
              className={tabStyle(selectedTab === 2)}
            >
              Projects
            </div>
            <div
              onClick={() => navigate("/events-admin")}
              className={tabStyle(selectedTab === 3)}
            >
              Events
            </div>
          </div>
        ...
        <div className="flex items-center mb-7">
          ...
          {hasNewNotifications && (
            <>
              {/* Dot for mobile size */}
              <span
                className="absolute top-8 right-[82px] w-2 h-2 bg-yellow-500 rounded-full 
                                block lg:hidden sm:w-3 sm:h-3"
              ></span>

              {/* Dot for website size */}
              <span
                className="absolute top-10 right-[207px] w-2 h-2 bg-yellow-500 rounded-full 
                                hidden lg:block"
              ></span>
            </>
          )}
          ...
          >
            ...
            />
            <span className="ml-2 text-gray-700 font-semibold hidden sm:inline">
              {userName}
            </span>
            {showLogoutBox && (
