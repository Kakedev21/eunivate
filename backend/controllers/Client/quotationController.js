...
export const createQuotation = async (req, res) => {
  ...
  try {
    ...
    const verificationLink = `https://eunivate-jys4.onrender.com/api/users/quotation/confirm/${quotationToken.quotationToken}`;
    ...
    try {
      ...
      res.status(201).json({
        message:
          "Quotation saved successfully, verification email sent, and completion email sent.",
        quotation,
      });
    } catch (emailError) {
      ...
      res.status(500).json({
        message:
          "Quotation saved but failed to send verification or completion email",
