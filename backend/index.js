...
app.get("/api/users/workspaces/selected", async (req, res) => {
  try {
    const selectedWorkspaceRecord = await SelectedWorkspace.findOne().populate(
      "selectedWorkspace"
    ); // Populate the workspace reference
    ...
    return res
      .status(500)
      .json({ error: error.message || "Error fetching selected workspace" });
  }
});
...
app.get("/quotation-complete", (req, res) => {
  ...
  res.redirect(`https://www.eunivate.com/quotation-complete`);
});
